jQuery(document).ready(function(e){var b;var d=false;var c=0;e(".cmb_timepicker").each(function(){e("#"+jQuery(this).attr("id")).timePicker({startTime:"07:00",endTime:"22:00",show24Hours:false,separator:":",step:30});
...
var j=true;var i=l.hasClass("cmb_upload_list");if(d){d.open();return;}d=wp.media.frames.file_frame=wp.media({title:e("label[for="+b+"]").text(),button:{text:window.cmb_l10.upload_file},multiple:i?true:false});
d.on("select",function(){if(i){j=d.state().get("selection").toJSON();k.val(j.url);e("#"+b+"_id").val(j.id);var m=[];e(j).each(function(){if(this.type&&this.type==="image"){g='<li class="img_status"><img width="50" height="50" src="'+this.url+'" class="attachment-50x50" alt="'+this.filename+'"><p><a href="#" class="cmb_remove_file_button" rel="'+b+"["+this.id+']">'+cmb_l10.remove_image+'</a></p><input type="hidden" id="filelist-'+this.id+'" name="'+b+"["+this.id+']" value="'+this.url+'"></li>';
...
}m.push(g);});e(m).each(function(){k.siblings(".cmb_media_status").slideDown().append(this);});}else{j=d.state().get("selection").first().toJSON();k.val(j.url);
...
}k.siblings(".cmb_media_status").slideDown().html(g);}});d.open();}).on("click",".cmb_remove_file_button",function(g){var i=e(this);if(i.is(".attach_list .cmb_remove_file_button")){i.parents("li").remove();
