...
setTimeout(e.resizeoEmbeds,500);d(c).on("resize",e.resizeoEmbeds);};e.handleMedia=function(h){if(typeof wp==="undefined"){return;}h.preventDefault();var m=e.metabox();
var n=d(this);e.formfield=n.prev("input").attr("id");var g=d("#"+e.formfield);var o=g.attr("name");var i=true;var l=true;var j=n.hasClass("cmb_upload_list");
if(e.formfield in e.file_frames){e.file_frames[e.formfield].open();return;}e.file_frames[e.formfield]=wp.media.frames.file_frame=wp.media({title:m.find("label[for="+e.formfield+"]").text(),button:{text:b.upload_file},multiple:j?true:false});
...
}m.length&&d(m).remove();});};d.fn.cleanRow=function(g,h){var i=d(this);if(h){i.find(".cmb-repeat-table .repeat-row:not(:first-child)").remove();}e.$focus=false;
e.neweditor_id=[];i.find('input:not([type="button"]),select,textarea,label').each(function(){var s=d(this);var k=s.hasClass("wp-editor-area");var l=s.attr("for");
var n=s.next();var t={};if(l){t={"for":l.replace("_"+g,"_"+e.idNumber)};}else{var j=s.attr("name");var r=j?j.replace(g,e.idNumber):"";var p=s.attr("id");
var q=p?p.replace("_"+g,"_"+e.idNumber):"";t={id:q,name:r,"data-iterator":e.idNumber,};}s.val("").removeAttr("checked").removeAttr("selected").removeClass("hasDatepicker").attr(t);
if(k){q=q?p.replace("zx"+g,"zx"+e.idNumber):"";s.html("");var m=s.parents(".cmb-type-wysiwyg");m.find(".mce-tinymce:not(:first-child)").remove();var o=m.html().replace(new RegExp(p,"g"),q);
m.html(o);e.neweditor_id.push({id:q,old:p});}e.$focus=e.$focus?e.$focus:s;});return this;};d.fn.newRowHousekeeping=function(){var g=d(this);var i=g.find(".wp-picker-container");
var h=g.find(".cmb_media_status");if(i.length){i.each(function(){var j=d(this).parent();j.html(j.find("input:text.cmb_colorpicker").attr("style",""));});
}if(h.length){h.empty();}return this;};e.afterRowInsert=function(j){if(e.$focus){e.$focus.focus();}if(e.neweditor_id.length){var l;for(l=e.neweditor_id.length-1;
...
}h.trigger("cmb_add_row",k);};e.addAjaxRow=function(g){g.preventDefault();var l=d(this);var m="#"+l.data("selector");var o=d(m);var h=o.find(".empty-row");
var j=parseInt(h.find("[data-iterator]").data("iterator"));e.idNumber=j+1;var k=h.clone();var n=k.find(".wp-picker-container");var i=k.find(".cmb_media_status");
k.newRowHousekeeping().cleanRow(j);h.removeClass("empty-row").addClass("repeat-row");h.after(k);e.afterRowInsert(k);o.trigger("cmb_add_row",k);};e.removeGroupRow=function(i){i.preventDefault();
var k=d(this);var h=d("#"+k.data("selector"));var j=k.parents(".repeatable-grouping");var g=h.find(".repeatable-grouping").length;if(g>1){j.remove();if(g<3){h.find(".remove-group-row").prop("disabled",true);
...
g.trigger("cmb_remove_row");}};e.initPickers=function(h,i,g){e.initTimePickers(h);e.initDatePickers(i);e.initColorPickers(g);};e.initTimePickers=function(g){if(!g.length){return;
