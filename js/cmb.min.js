...
var p=j?"list":"single";k[p](q);});e.file_frames[e.formfield].open();};e.handleRemoveMedia=function(g){var i=d(this);if(i.is(".attach_list .cmb_remove_file_button")){i.parents("li").remove();
return false;}e.formfield=i.attr("rel");var h=i.parents(".img_status");$metabox.find("input#"+e.formfield).val("");$metabox.find("input#"+e.formfield+"_id").val("");
if(!h.length){i.parents(".cmb_media_status").html("");}else{h.html("");}return false;};e.addAjaxRow=function(i){i.preventDefault();var q=d(this);var s="#"+q.data("selector");
var g=s.slice(1,s.lastIndexOf("repeat"));var t=d(s).data("numberels",e.idNumber);var k=t.find(".empty-row");e.idNumber=parseInt(k.find("[data-iterator]").data("iterator"))+1;
var n=k.clone();var m=e.idNumber-1;var r=n.find(".wp-picker-container");var l=n.find(".cmb_media_status");var p,h;if(r.length){var j=n.find("td:first-child");
j.html(j.find("input:text.cmb_colorpicker"));}if(l.length){l.empty();}var o=function(){p=d(this);h=h?h:p;var x=p.attr("name");var v=x?x.replace(m,e.idNumber):"";
var y=p.attr("id");var w=y?y.replace("_"+m,"_"+e.idNumber):"";var u=p.next();e.log(p.prop("tagName"),x,v,m,e.idNumber);p.val("").removeAttr("checked").removeAttr("selected").removeClass("hasDatepicker").attr({id:w,name:v,"data-iterator":e.idNumber,});
if(u.is("label")){u.attr("for",w);}};n.find('input:not([type="button"]),select,textarea').each(o);k.removeClass("empty-row").addClass("repeat-row");k.after(n);
if(h){h.focus();}e.initPickers(n.find("input:text.cmb_timepicker"),n.find("input:text.cmb_datepicker"),n.find("input:text.cmb_colorpicker"));};e.removeAjaxRow=function(h){h.preventDefault();
var j=d(this);var i=j.parents("tr");var g=j.parents(".cmb-repeat-table");if(g.find("tr").length>1){if(i.hasClass("empty-row")){i.prev().addClass("empty-row").removeClass("repeat-row");
