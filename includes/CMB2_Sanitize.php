...
class CMB2_Sanitize {
	...
	public function __construct( CMB2_Field $field, $value ) {
		$this->field = $field;
		$this->value
		             = stripslashes_deep( $value ); // get rid of those evil magic quotes
	}
	...
	public function default_sanitization() {
		 ...
		 */
		$override_value = apply_filters( "cmb2_sanitize_{$this->field->type()}",
			null, $this->value, $this->field->object_id, $this->field->args(),
			$this );
		/**
		...
		$override_value = apply_filters( "cmb2_validate_{$this->field->type()}",
			$override_value, $this->value, $this->field->object_id,
		...
		switch ( $this->field->type() ) {
			...
			case 'taxonomy_multicheck':
				if ( $this->field->args( 'taxonomy' ) ) {
					wp_set_object_terms( $this->field->object_id, $this->value,
						$this->field->args( 'taxonomy' ) );
					break;
				}
			case 'multicheck':
			case 'file_list':
			...
			default:
				...
				$sanitized_value = is_array( $this->value )
					? array_map( 'sanitize_text_field', $this->value )
					: call_user_func( 'sanitize_text_field', $this->value );
				break;
		}

		return $this->_is_empty_array( $sanitized_value ) ? ''
			: $sanitized_value;
	}
	...
	public function text_url() {
		...
		if ( is_array( $this->value ) ) {
			foreach ( $this->value as $key => $val ) {
				$this->value[ $key ] = $val ? esc_url_raw( $val, $protocols )
					: $this->field->args( 'default' );
			}
		} else {
			$this->value = $this->value ? esc_url_raw( $this->value,
				$protocols ) : $this->field->args( 'default' );
		}
	...
	public function text_money() {
		...
		if ( is_array( $this->value ) ) {
			foreach ( $this->value as $key => $val ) {
				$this->value[ $key ]
					= number_format_i18n( (float) str_ireplace( $search,
					$replace, $val ), 2 );
			}
		} else {
			$this->value = number_format_i18n( (float) str_ireplace( $search,
				$replace, $this->value ), 2 );
		}
	...
	public function text_date_timestamp() {
		return is_array( $this->value ) ? array_map( 'strtotime', $this->value )
	...
	public function text_datetime_timestamp( $repeat = false ) {
		...
		$this->value = strtotime( $this->value['date'] . ' '
		                          . $this->value['time'] );
	...
	public function text_datetime_timestamp_timezone( $repeat = false ) {
		...
		if ( is_array( $this->value )
		     && array_key_exists( 'timezone', $this->value )
		) {
		...
		$this->value = new DateTime( $this->value['date'] . ' '
		                             . $this->value['time'],
	...
	public function textarea() {
		return is_array( $this->value ) ? array_map( 'wp_kses_post',
	...
	public function _save_file_id() {
		...
		if ( $group ) {
			...
			$i       = $group->index;
			$base_id = $group->_id();
			$id_val  = isset( $_POST[ $base_id ][ $i ][ $id_key ] )
				? absint( $_POST[ $base_id ][ $i ][ $id_key ] ) : 0;

		} else {
			// Check standard $_POST data
			$id_val = isset( $_POST[ $field->id() ] ) ? $_POST[ $field->id() ]
				: null;
	...
	public function file() {
		...
		return $this->field->group ? array_merge( array( 'url' => $clean ),
			$id_value ) : $clean;
	}
