...
class CMB_Customize_Radio_Taxonomy extends WP_Customize_Control {
    ...
    public function render_content() {       
       ...
       foreach( $terms as $index => $term ) {
           printf( '<label for="%1$s"><input type="radio" value="%2$s" id="%1$s" name="%3$s" %4$s data-customize-setting-link="%6$s" />&nbsp;%5$s</label></li>', esc_attr( $this->id . '_' . $counter ), esc_html( $term->term_id ), '_' . esc_attr( $this->id ), checked( $term->term_id, $this->value(), false ), esc_html( $term->name ), esc_attr( $this->id ) );
           $counter += 1;   
       }
       echo '</div>';
    }
