...
class CMB2_Type_Wysiwyg extends CMB2_Type_Textarea {
	...
	public function add_wysiwyg_template_for_group() {
		...
		$editor = $this->get_wp_editor( array(
			'value'   => 'cmb2_replace_value_'. $group_id,
			'id'      => 'cmb2_replace_id_'. $group_id,
			'options' => $options,
		) );

		// Then replace the special id/value/name with underscore placeholders.
		$editor = str_replace( array(
			'cmb2_replace_name_'. $group_id,
			'cmb2_replace_value_'. $group_id,
			'cmb2_replace_id_'. $group_id,
		), array(
			'{{ data.name }}',
			'{{{ data.value }}}',
			'{{ data.id }}',
		...
		echo '<script type="text/template" id="tmpl-cmb2-wysiwyg-'. $group_id .'">';
		// Need to wrap the template in a wrapper div w/ specific data attributes which will be used when adding/removing rows.
		echo '<div class="cmb2-wysiwyg-inner-wrap" data-iterator="{{ data.iterator }}" data-groupid="'. $group_id .'" data-id="'. $this->field->id( true ) .'">'. $editor .'</div>';
		echo '</script>';
	}
